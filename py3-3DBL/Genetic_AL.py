...
class Chromo(Solution):
    ...
    def initialpop(cls,Data,nPop,Varsize,crossover_rate,mutation_rate,mu):
        ...
        while len(cls.pop) < nPop:
            order=list(range(Varsize))
            shuffle( order )
            sol = cls( order )
            if order not in Chromo.Listofsolutions:
                Chromo.Listofsolutions.append(order)
                cls.pop.append(sol.Score_Calc(Data, Chromo.alpha , Chromo.beta , Chromo.gamma))
    @classmethod
    def pop_sort(cls):
        pp=cls.pop
    ...
    def crossover(cls, Dad_value, Mom_value):
        ...
        for j in range(y+1, cls.Varsize):
    ...
    def evolve(cls,Data,TotalNoSolution):
       ...
       ###################################################################         
        children=[x.Score_Calc(Data,  Chromo.alpha , Chromo.beta , Chromo.gamma) for x in children]
        Mutants=[x.Score_Calc(Data,  Chromo.alpha , Chromo.beta , Chromo.gamma) for x in Mutants]
